pub use expj_protocol::ErrorCode;
use expj_protocol::{
...
pub struct ExpjMetrics(Arc<MetricsInner>);
...
impl Telemetry for ExpjMetrics {
    fn start_request(&self, info: RequestInfo) -> Option<Box<dyn RequestObservation>> {
    ...
    async fn dispatch(&self, mut frame: Frame) -> Frame {
        ...
        let Some(handler) = self.handlers.get(&frame.method_id) else {
            tracing::debug!(
                method_id = frame.method_id,
                request_id = frame.request_id,
                "EXPJ request used an unknown method"
            );
            finish_observation(
                observation,
                RequestOutcome::Error {
            ...
            Some(duration) => match timeout(duration, handler(payload)).await {
                ...
                Err(_) => {
                    tracing::debug!(
                        method_id = frame.method_id,
                        request_id = frame.request_id,
                        "EXPJ request exceeded its deadline"
                    );
                    finish_observation(
                        observation,
                        RequestOutcome::Error {
            ...
            Err(error) => {
                if error.code == ErrorCode::Internal {
                    tracing::error!(
                        ...
                        "EXPJ handler returned an internal error"
                    );
                } else {
                    tracing::debug!(
                        method_id = frame.method_id,
    ...
    pub async fn run(self) -> io::Result<()> {
        match self.listener {
            Listener::Tcp(listener) => loop {
                ...
                tokio::spawn(async move {
                    if let Err(error) = serve_connection(stream, router, settings).await {
                        tracing::debug!(%error, "EXPJ TCP connection closed");
                    }
                });
            },
            Listener::Tls(listener, acceptor) => loop {
                ...
                tokio::spawn(async move {
                    ...
                    match handshake {
                        Ok(Ok(stream)) => {
                            if let Err(error) = serve_connection(stream, router, settings).await {
                                tracing::debug!(%error, "EXPJ TLS connection closed");
                            }
                        }
                        Ok(Err(error)) => tracing::warn!(%error, "EXPJ TLS handshake rejected"),
                        Err(_) => tracing::warn!("EXPJ TLS handshake timed out"),
                    }
                });
            },
            #[cfg(unix)]
            Listener::Unix(listener) => loop {
                ...
                tokio::spawn(async move {
                    if let Err(error) = serve_connection(stream, router, settings).await {
                        tracing::debug!(%error, "EXPJ Unix connection closed");
                    }
                });
            },
        }
