...
import dev.expj.client.ExpjChannel;
...
public final class MoonLightBridge implements AutoCloseable {
    ...
    private final ReconnectingExpjClient client;
    private final ExecutorService background = Executors.newVirtualThreadPerTaskExecutor();
    private final AtomicBoolean closed = new AtomicBoolean();

    private MoonLightBridge(Plugin plugin, ReconnectingExpjClient client) {
    ...
    public static MoonLightBridge start(Plugin plugin, String endpoint) throws IOException {
        return new MoonLightBridge(plugin, ReconnectingExpjClient.start(endpoint));
    }

    public ExpjChannel channel() { return client; }
