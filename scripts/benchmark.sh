...
"$project_dir/target/release/moonlight-bridge-example-backend" >"$project_dir/moonlight-bridge-benchmark.log" 2>&1 &
backend_pid=$!
for _ in {1..50}; do
...
MOONLIGHT_BRIDGE_UNIX_PATH="$socket_path" "$project_dir/target/release/moonlight-bridge-example-backend" \
    >>"$project_dir/moonlight-bridge-benchmark.log" 2>&1 &
backend_pid=$!
for _ in {1..50}; do
